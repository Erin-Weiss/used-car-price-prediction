...
KERAS_LATEST_PATH = KERAS_DIR / "keras_model.keras"      # "latest" pointer
...
def clipped_mask_by_y_true(y_true, q_low=0.01, q_high=0.99):
    ...
    Clipping is based ONLY on y_true (never on predictions).
    """
    y_true = np.asarray(y_true)
...
def metrics_to_df(metrics_dict, model_name: str) -> pd.DataFrame:
    """
    Convert output from metrics_full_and_clipped into a pretty DataFrame.
    """
    rows = []
...
def show_metrics_table(name, y_true_dollars, y_pred_dollars):
    m = metrics_full_and_clipped(y_true_dollars, y_pred_dollars, 0.01, 0.99)
    df = pd.DataFrame([
        {"model": name, "view": "full", **m["full"]},
        {"model": name, "view": "clipped", **m["clipped"]},
    ]).set_index(["model", "view"])
    display(df)
    return m
```
...
if "TRAIN_MODE" in globals() and TRAIN_MODE == "skip":
    ...
    display(metrics_to_df(ridge_m, "Ridge"))
...
if final_cb is None:
    ...
    # ----------------------------
    # NEW: TRAIN split metrics
    # ----------------------------
    cb_pred_log_train = final_cb.predict(X_train)
    cb_pred_train = np.expm1(cb_pred_log_train)
    cb_true_train = np.expm1(y_train)

    cb_mae_train = mean_absolute_error(cb_true_train, cb_pred_train)
    cb_rmse_train = np.sqrt(mean_squared_error(cb_true_train, cb_pred_train))

    print(f"CATBOOST TRAIN MAE ($):  {cb_mae_train:,.0f}")
    print(f"CATBOOST TRAIN RMSE ($): {cb_rmse_train:,.0f}")
    ...
    # ----------------------------
    # NEW: VAL split metrics (early-stopping set)
    # ----------------------------
    cb_pred_log_val = final_cb.predict(X_val)
    cb_pred_val = np.expm1(cb_pred_log_val)
    cb_true_val = np.expm1(y_val)

    cb_mae_val = mean_absolute_error(cb_true_val, cb_pred_val)
    cb_rmse_val = np.sqrt(mean_squared_error(cb_true_val, cb_pred_val))

    print(f"CATBOOST VAL MAE ($):  {cb_mae_val:,.0f}")
    print(f"CATBOOST VAL RMSE ($): {cb_rmse_val:,.0f}")

    cb_v = metrics_full_and_clipped(cb_true_val, cb_pred_val, q_low=0.01, q_high=0.99)
    display(metrics_to_df(cb_v, "CatBoost (Val)"))

    # ----------------------------
    # EXISTING: TEST split metrics
    # ----------------------------
    # Predict
    ...
    display(metrics_to_df(cb_m, "CatBoost"))

    # ----------------------------
    # EXISTING: Train+Val metrics for comparison
    # ----------------------------
...
# ----------------------------
# Step 2: Prepare train/val/test (simple, explicit)
# - numeric: fill NaNs with train medians
# - categorical: fill NaNs with "unknown" and cast to string
# ----------------------------
X_train_nn = X_train.copy()
X_val_nn   = X_val.copy()
X_test_nn  = X_test.copy()

# Add engine missing indicators (helps NNs; optional but simple)
for col in ["engine_liters", "engine_cylinders"]:
    if col in X_train_nn.columns:
        miss = f"{col}_missing"
        X_train_nn[miss] = X_train_nn[col].isna().astype("int64")
        X_val_nn[miss]   = X_val_nn[col].isna().astype("int64")
        X_test_nn[miss]  = X_test_nn[col].isna().astype("int64")
        if miss not in num_cols:
            num_cols.append(miss)

# numeric impute using TRAIN medians only
train_medians = X_train_nn[num_cols].median(numeric_only=True)
X_train_nn[num_cols] = X_train_nn[num_cols].fillna(train_medians)
X_val_nn[num_cols]   = X_val_nn[num_cols].fillna(train_medians)
X_test_nn[num_cols]  = X_test_nn[num_cols].fillna(train_medians)
...
```{python}
# ----------------------------
# Step 3: Build preprocessing layers (readable)
# - Normalization for numeric
# - StringLookup + Embedding per categorical feature
# ----------------------------

# ----------------------------
# Keras: decide whether we will TRAIN (and thus need adapt/build)
# ----------------------------
...
if not WILL_TRAIN_KERAS:
    ...
    # 3A) Numeric normalization
    num_norm = layers.Normalization(name="num_norm")
    num_norm.adapt(X_train_nn[num_cols].astype("float32").to_numpy())

    # 3B) Categorical lookups + embedding sizes
    lookups = {}
    embed_dims = {}
    vocab_sizes = {}
...
```{python}
# ----------------------------
# Step 4: Build the Keras model (clear architecture)
# Activation choice:
# - SiLU/Swish is a strong default for tabular (often smoother than ReLU)
# - You can switch to 'relu' easily if you want
# ----------------------------

if not WILL_TRAIN_KERAS:
    print("⏭️ Skipping model build (will load existing Keras model later).")
else:
    # Inputs
    numeric_in = keras.Input(shape=(len(num_cols),), name="numeric", dtype=tf.float32)
    cat_inputs = {c: keras.Input(shape=(1,), name=c, dtype=tf.string) for c in cat_cols}

    # Numeric path
    x_num = num_norm(numeric_in)

    # Categorical paths -> lookup -> embedding -> flatten
    emb_list = []
    for c in cat_cols:
        x = lookups[c](cat_inputs[c])
        x = layers.Embedding(input_dim=vocab_sizes[c], output_dim=embed_dims[c], name=f"{c}_emb")(x)
        x = layers.Reshape((embed_dims[c],), name=f"{c}_emb_flat")(x)
        emb_list.append(x)
    ...
    x = layers.Activation(tf.keras.activations.silu)(x)
    ...
    out = layers.Dense(1, name="pred_log1p")(x)

    model = keras.Model(
        inputs={"numeric": numeric_in, **cat_inputs},
        outputs=out,
        name="tabular_nn_embeddings"
    )
...
# ----------------------------
# Step 5: Convert pandas -> Keras-friendly input dicts
# ----------------------------
def to_input_dict(Xdf: pd.DataFrame):
    d = {"numeric": Xdf[num_cols].astype("float32").to_numpy()}
    for c in cat_cols:
        d[c] = Xdf[c].astype(str).to_numpy()
    return d
...
```{python}
# ----------------------------
# Step 6: Train or Load (EarlyStopping + ReduceLROnPlateau + checkpoint)
# ----------------------------

# Expect these globals already exist from your earlier setup:
# - TRAIN_MODE in {"skip","smoke","full"}
# - RETRAIN_KERAS: bool
# - KERAS_DIR, KERAS_RUNS_DIR created earlier (or we create them here safely)
# - model already built/compiled if we're going to train
# - train_inputs, val_inputs, y_train_nn, y_val_nn exist if we're going to train

# ----------------------------
# Step 6: Train or Load
# ----------------------------

latest_path = KERAS_LATEST_PATH
KERAS_RUNS_DIR = KERAS_RUNS_DIR
KERAS_DIR = KERAS_DIR 
...
# 3) Otherwise TRAIN (smoke always trains; full trains if retraining or missing latest)
else:
    if model is None:
        raise RuntimeError(
            "Keras training requested, but `model` is None. "
            "Make sure Step 4 ran (model built/compiled) before Step 6."
        )

    if TRAIN_MODE == "smoke":
        print("🧪 Smoke mode: training a quick sanity-check run (will NOT update latest).")
        EPOCHS = 10
        BATCH = 1024
    else:
        print("🔁 Training Keras (full).")
        EPOCHS = 50
        BATCH = 2048
    ...
    best_path = KERAS_RUN_DIR / "best.keras"
    versioned_path = KERAS_RUN_DIR / "keras_model.keras"

    callbacks = [
        keras.callbacks.EarlyStopping(
            monitor="val_loss",
            patience=8 if TRAIN_MODE == "full" else 3,
            restore_best_weights=True
        ),
        keras.callbacks.ReduceLROnPlateau(
            monitor="val_loss",
            factor=0.5,
            patience=3 if TRAIN_MODE == "full" else 1,
            min_lr=1e-6
        ),
    ...
    # ----------------------------
    print("TRAIN_MODE =", TRAIN_MODE, "| epochs=", EPOCHS, "| batch=", BATCH)
    print("train rows =", len(X_train_nn), "val rows =", len(X_val_nn))
    # ----------------------------

    # ✅ ACTUAL TRAINING (now reachable)
    history = model.fit(
        train_inputs, y_train_nn,
        validation_data=(val_inputs, y_val_nn),
        epochs=EPOCHS,
        batch_size=BATCH,
        verbose=2,
        callbacks=callbacks
    ...
    print("KERAS_RUN_DIR =", KERAS_RUN_DIR)
    # ----------------------------
    ...
    atomic_write_json(KERAS_RUN_DIR / "run_info.json", {
        "train_mode": TRAIN_MODE,
        "timestamp": run_ts,
    ...
    atomic_write_json(KERAS_RUN_DIR / "train_medians.json", {
...
if ("model" not in globals()) or (model is None):
    ...
    # Test
    pred_log_test = model.predict(test_inputs, batch_size=4096).ravel()
    pred_test = np.expm1(pred_log_test)
    true_test = np.expm1(y_test_nn)

    keras_test_m = show_metrics_table("Keras", true_test, pred_test)
    ...
    pred_log_trainval = model.predict(trainval_inputs, batch_size=4096, verbose=0).ravel()
    pred_trainval = np.expm1(pred_log_trainval)
    true_trainval = np.expm1(y_trainval_nn)

    keras_trainval_m = show_metrics_table("Keras (Train+Val)", true_trainval, pred_trainval)

    # Optional globals for your JSON metrics writer
    keras_mae_test = keras_test_m["full"]["mae"]
    keras_rmse_test = keras_test_m["full"]["rmse"]
    keras_mae_trainval = keras_trainval_m["full"]["mae"]
    keras_rmse_trainval = keras_trainval_m["full"]["rmse"]
    keras_test_clipped = keras_test_m["clipped"]
    keras_trainval_clipped = keras_trainval_m["clipped"]

    print("✅ Done: Keras NN trained and evaluated.")
```

### Metrics Savepoint
```{python}
# -------------------------
# Save Metrics (record-only; NO re-evaluation here)
# -------------------------
...
metrics["_meta"] = {
    ...
    "clip_policy": {
        ...
        "q_low": 0.01,
        "q_high": 0.99,
        "note": "Clipped metrics computed on subsets where true price is within [q_low, q_high] quantiles; predictions are never used for clipping.",
    },
}

# ---------- Ridge metrics (record-only) ----------
...
if ridge_vars_exist:
    ...
    # Optional: include clipped if you computed these earlier
    if "ridge_test_clipped" in globals() and ridge_test_clipped is not None:
        metrics["ridge"]["test_clipped"] = ridge_test_clipped
    if "ridge_trainval_clipped" in globals() and ridge_trainval_clipped is not None:
        metrics["ridge"]["trainval_clipped"] = ridge_trainval_clipped

else:
    print("ℹ️ Skipping Ridge metrics write (need ridge_mae_trainval/ridge_rmse_trainval/ridge_mae_test/ridge_rmse_test).")

# ---------- CatBoost metrics (record-only) ----------
...
if cb_vars_exist:
    ...
    # Optional: include CV-best val metrics if you computed them earlier
    ...
    # Optional: include clipped if you computed these earlier
    if "cb_test_clipped" in globals() and cb_test_clipped is not None:
        metrics["catboost"]["test_clipped"] = cb_test_clipped
    if "cb_trainval_clipped" in globals() and cb_trainval_clipped is not None:
        metrics["catboost"]["trainval_clipped"] = cb_trainval_clipped

else:
    print("ℹ️ Skipping CatBoost metrics write (need cb_mae_trainval/cb_rmse_trainval/cb_mae_test/cb_rmse_test).")

# ---------- Keras metrics (record-only) ----------
# Your Keras evaluation cell already sets:
#   keras_mae_test, keras_rmse_test, keras_mae_trainval, keras_rmse_trainval
#   keras_test_clipped, keras_trainval_clipped
keras_vars_exist = all(name in globals() for name in [
    "keras_mae_trainval", "keras_rmse_trainval", "keras_mae_test", "keras_rmse_test"
])

if keras_vars_exist:
    metrics["keras"] = {
        "trainval_mae": float(keras_mae_trainval),
        "trainval_rmse": float(keras_rmse_trainval),
        "test_mae": float(keras_mae_test),
        "test_rmse": float(keras_rmse_test),
        "run_dir": str(KERAS_RUN_DIR) if "KERAS_RUN_DIR" in globals() else None,
    }

    # Optional: include clipped metrics if present
    if "keras_test_clipped" in globals() and keras_test_clipped is not None:
        metrics["keras"]["test_clipped"] = keras_test_clipped
    if "keras_trainval_clipped" in globals() and keras_trainval_clipped is not None:
        metrics["keras"]["trainval_clipped"] = keras_trainval_clipped

else:
    print("ℹ️ Skipping Keras metrics write (need keras_mae_trainval/keras_rmse_trainval/keras_mae_test/keras_rmse_test).")
...
if "KERAS_RUN_DIR" in globals() and KERAS_RUN_DIR is not None:
    keras_only = {
        "_meta": metrics["_meta"],
        "keras": metrics.get("keras", None),
    }
    atomic_write_json(KERAS_RUN_DIR / "keras_metrics.json", keras_only)
    print("🧾 Wrote Keras-only metrics:", KERAS_RUN_DIR / "keras_metrics.json")
