...
```{python}
# ----------------------------
# Step -1: Simple Keras config (start stable)
# ----------------------------
KERAS_CFG = {
    "batch_full": 2048, # 2048 orginally 
    "epochs_full": 80,          # EarlyStopping will stop earlier
    "batch_smoke": 512,
    "epochs_smoke": 3,
    ...
    # optional smoke subsample (stable and simple)
    "smoke_subsample": False,
    "smoke_train_n": 15000,
    "smoke_val_n": 5000,
}
```
...
```{python}
# ----------------------------
# Step 1: Define feature columns (base)
# ----------------------------
NUM_COLS_BASE = [
    "mileage", "mpg_avg", "price_drop", "seller_rating", "driver_rating",
    "driver_reviews_num", "accidents_or_damage", "one_owner", "personal_use_only",
...
print("Numeric cols:", len(num_cols_nn))
print("Categorical cols:", len(cat_cols_nn))
...
# Add missing indicators (optional but helpful)
...
# IMPORTANT: keep as (N,) for simplicity
...
print("WILL_TRAIN_KERAS =", WILL_TRAIN_KERAS, "| latest exists =", latest_path.exists())
...
if WILL_TRAIN_KERAS:
    ...
    for c in cat_cols_nn:
        ...
        vs = len(vocab)
        vocab_sizes[c] = vs
        EMBED_DIMS[c] = suggest_embed_dim(vs)
...
def to_input_dict_tf(inputs_np: dict) -> dict:
    """
    numeric -> tf.float32 (N, D)
    cats -> tf.string (N,)  IMPORTANT: rank-1
    """
    out = {"numeric": tf.convert_to_tensor(inputs_np["numeric"], dtype=tf.float32)}
    for c in cat_cols_nn:
        out[c] = tf.convert_to_tensor(inputs_np[c].tolist(), dtype=tf.string)  # (N,)
    return out
...
def build_baseline_model():
    ...
    # Fresh preprocessing layers (re-built from frozen state)
    ...
    # Cat lookups + embeddings (each embedding output is (N, emb_dim))
    emb_list = []
    for c in cat_cols_nn:
        lk = layers.StringLookup(
            output_mode="int",
        ...
        emb = layers.Dropout(0.20)(emb)
        emb_list.append(emb)

    x = layers.Concatenate(name="concat")([x_num] + emb_list)

    # Dense stack (same spirit as your working model)
    ...
    model = keras.Model(inputs={"numeric": numeric_in, **cat_in}, outputs=out, name="tabular_nn_baseline")

# , weight_decay=1e-4,clipnorm=1.0

    model.compile(
        optimizer=keras.optimizers.Adam(learning_rate=KERAS_CFG["lr"]),
...
def make_callbacks_rlrop(train_mode: str):
    ES_PAT = KERAS_CFG["es_patience_smoke"] if train_mode == "smoke" else KERAS_CFG["es_patience_full"]
    RL_PAT = KERAS_CFG["rlrop_patience_smoke"] if train_mode == "smoke" else KERAS_CFG["rlrop_patience_full"]
    return [
        # keras.callbacks.ReduceLROnPlateau(
        #     monitor="val_loss",
        #     factor=KERAS_CFG["rlrop_factor"],
        #     patience=RL_PAT,
        #     min_lr=KERAS_CFG["min_lr"],
        #     verbose=0,
        # ),
...
# 1) SKIP: never train; load latest if it exists
if TRAIN_MODE == "skip":
...
# 2) FULL + no retrain: load latest if it exists
...
# 3) Otherwise: TRAIN (smoke always trains; full trains if retrain or missing latest)
else:
    ...
    # Mode-specific knobs
    # ----------------------------
    BATCH  = KERAS_CFG["batch_smoke"] if TRAIN_MODE == "smoke" else KERAS_CFG["batch_full"]
    EPOCHS = KERAS_CFG["epochs_smoke"] if TRAIN_MODE == "smoke" else KERAS_CFG["epochs_full"]

    # optional smoke subsample (stable)
    if TRAIN_MODE == "smoke" and KERAS_CFG.get("smoke_subsample", False):
        rs = np.random.RandomState(SEED)
        ntr = min(int(KERAS_CFG["smoke_train_n"]), len(y_train_nn))
        nva = min(int(KERAS_CFG["smoke_val_n"]), len(y_val_nn))
    ...
    best_weights_path = str(KERAS_RUN_DIR / "best.weights.h5")
    versioned_path = KERAS_RUN_DIR / "keras_model.keras"

    # write run_info (matches your style)
    run_info = {
        "run_ts": run_ts,
        "train_mode": TRAIN_MODE,
        ...
        "keras_cfg": dict(KERAS_CFG),
    }
    atomic_write_json(KERAS_RUN_DIR / "run_info.json", run_info)

    # keep the imputation medians used
    atomic_write_json(
    ...
    print(f"🚆 Training Keras | mode={TRAIN_MODE} | epochs={EPOCHS} | batch={BATCH}")
    history = model.fit(
        train_inputs_run, y_train_run,
        validation_data=(val_inputs_run, y_val_run),
        epochs=EPOCHS,
        batch_size=min(int(BATCH), int(len(y_train_run))),  # safety if smoke tiny
        verbose=0,
        callbacks=make_callbacks_rlrop(TRAIN_MODE),
    )
...
if ("model" not in globals()) or (model is None):
    ...
    def predict_dollars(inputs_tf: dict) -> np.ndarray:
        pred_log = model.predict(inputs_tf, batch_size=KERAS_CFG["pred_batch"], verbose=0).reshape(-1)
