...
def build_ft_transformer_model(lr_schedule):
    ...
    # Numeric tokenization
    # num_tokens = 1 -> current behavior
    # num_tokens > 1 -> learned k-token projection
    if num_tokens == 1:
        num_token_block = layers.Dense(d_token, name="num_token_proj")(x_num)
        num_token_block = layers.Reshape((1, d_token), name="num_token_reshape")(num_token_block)
    else:
    ...
    for c in cat_cols_nn:
        ...
        emb = layers.Reshape((1, d_token), name=f"{c}_emb_reshape")(emb)  # (N,1,d_token)
        cat_tokens.append(emb)

    # Token sequence: numeric tokens + categorical tokens => (N, T, d_token)
    ...
    # Head
    x = layers.Dense(d_token, activation=tf.keras.activations.gelu)(x)
