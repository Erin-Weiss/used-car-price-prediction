...
This project predicts used-car listing prices from structured vehicle attributes using three progressively complex modeling approaches. The goal is not simply to minimize error, but to understand the tradeoffs between interpretability, feature engineering effort, and predictive performance on real-world tabular data.

::: {.callout-note title="Why This Matters"}
Used-car pricing is a high-stakes, high-volume problem. Dealerships, online marketplaces, and lenders all need fast, accurate price estimates to set competitive listing prices, flag underpriced inventory for acquisition, detect overpriced listings, and underwrite auto loans. A model that predicts within ~$1,300 of the true price at the median — as CatBoost does here — is accurate enough to automate first-pass pricing, reduce the need for manual appraisals, and surface arbitrage opportunities across a portfolio of thousands of vehicles.
:::

The final selected model is **CatBoost**, which achieved the best generalization performance across all metrics while also requiring the least hyperparameter sensitivity and producing the most stable predictions across the full price range.
...
- **Ridge regression** — an interpretable linear baseline that establishes a performance floor and reveals which engineered features carry signal in a simple additive framework.
...
This progression allows direct comparison between classical ML and deep learning approaches on the same feature set, using the same train/validation/test split.

### Model Comparison Summary

| Metric | Ridge | CatBoost | FT-Transformer |
|---|---|---|---|
| **Test RMSE (full)** | $7,827 | **$3,940** | $4,196 |
| **Test MAE (full)** | $3,013 | **$1,912** | $1,928 |
| **Test MedianAE (full)** | $1,758 | **$1,278** | $1,302 |
| Test RMSE (clipped 1–99%) | $4,214 | **$2,659** | $2,691 |
| Test MAE (clipped 1–99%) | $2,655 | **$1,783** | $1,803 |

CatBoost outperforms Ridge by roughly 50% on every metric and edges out the FT-Transformer by a meaningful margin, particularly on full-distribution RMSE where sensitivity to rare high-value vehicles amplifies differences.

### Key Predictive Features

CatBoost feature importance analysis highlights several dominant predictors of vehicle price.
...
Engine characteristics such as cylinder count and displacement also contribute predictive signal, reflecting differences in vehicle performance and market positioning.

Overall, the model captures both **vehicle condition factors (age, mileage)** and **brand hierarchy (manufacturer, model)** when estimating price.
...
The FT-Transformer performed competitively on clipped metrics (within ~$30 of CatBoost on clipped MAE), but its full-distribution RMSE was ~6% higher, indicating greater sensitivity to rare, high-value vehicles. For a production pricing tool where robustness across the full price range matters, CatBoost is the stronger choice.
...
Because used-car prices are highly skewed (median ~$27k but a long tail past $100k reaching $1.9M), metrics are reported in two views:

- **Full distribution** — all test rows, which reflects real-world performance including rare luxury vehicles.
- **Clipped (1–99% of true prices)** — excludes the most extreme ~2% of listings to show performance on typical consumer vehicles.

Both views are important: the full distribution reveals how models handle tail risk, while the clipped view reflects the experience of a typical user.
...
#### Metric Reporting Philosophy

Because used-car prices are highly skewed, RMSE is dominated by rare luxury vehicles priced far above the median market. In addition to full-distribution metrics, I report 1–99% clipped metrics based on training-set price quantiles to better reflect typical consumer vehicles. Median absolute error remains stable across both views, indicating that most predictions are robust.

The gap between full-distribution RMSE and clipped RMSE (e.g., $3,940 vs. $2,659 for CatBoost) quantifies the impact of tail observations and provides a more complete picture than any single metric.
...
Model training is controlled via explicit retraining flags. By default, the pipeline loads existing checkpoints for fast iteration. When experimentation is needed, a retrain flag forces fresh training while preserving prior model versions with timestamped artifacts.

### Data Pipeline Versioning

Cleaning and feature engineering results are cached as Parquet files. A version flag (`FEATURE_PIPELINE_VERSION`) controls cache invalidation. When feature logic changes, the version is incremented so stale artifacts are automatically rebuilt.
...
>     - `train_medians.json` (latest numeric imputation values)
>     - `num_norm_stats.json` (latest normalization statistics)
>     - `lookup_vocabs.json` (latest categorical vocabularies)
>     - `runs/`
>       - `run_{timestamp}_{train_mode}/` (created)
>         - `keras_model.weights.h5` (versioned trained weights)
>         - `run_info.json` (written)
>         - `best_hp.json` (written if tuning enabled)
...
>
> - `notes/` (development notes; not part of the pipeline)
> - `reports/`
> - `scripts/`
> - `src/`
...
Artifact management and reproducibility are handled through a lightweight versioning system.  
The variable `FEATURE_PIPELINE_VERSION` controls the version of cleaned datasets, engineered feature tables, and the shared train/validation/test split indices. When data cleaning or feature engineering logic changes, this version is incremented so cached artifacts are automatically regenerated and stale datasets are not reused.

Each model directory stores both **versioned run artifacts** (`run_{timestamp}_{train_mode}`) and **latest pointers** used for evaluation and `TRAIN_MODE="skip"` workflows. Versioned runs preserve the full history of experiments, while the latest pointer provides a stable reference for downstream evaluation.

Additional metadata files (such as `ridge_run_info.json` and `catboost_run_info.json`) record the expected feature schema and pipeline version. When loading saved models, the code validates that the current feature pipeline matches the schema used during training, preventing accidental evaluation of models against incompatible feature sets.

### Project Setup

The setup cell loads all dependencies, defines project paths, configures training control flags, and includes a Kaggle CLI download helper that automatically fetches the raw dataset if it's not already present locally. All artifact paths follow the versioning system described in the folder structure above.
```{python}
...
# Commented out for notebook, uncomment if you want project root printed
...
def ensure_raw_csv(
    ...
    *,
    ...
    if raw_csv_path.exists() and not force:
        # Commented out for notebook, uncomment if you want raw dataset path printed
...
def load_or_build(path: Path, build_fn, *, label: str):
    ...
    """
    if path.exists():
        # Commenting out for notebook, but can switch print for debugging
...
Adds the project root to the Python path so modules in `src/` can be imported if needed.
```{python}
#| echo: false
...
The core cleaning and feature engineering functions are exported to `src/pipeline.py` as a standalone module. This allows the same logic to be used outside the notebook (e.g., in a CLI training script) without duplicating code. The notebook remains the primary reference version.
```{python}
#| echo: false
# -------------------------------------------------------
# Optional: export preprocessing logic to src/pipeline.py
# -------------------------------------------------------
...
# For readers interested in a modular project structure,
# the core cleaning and feature engineering functions are
# also exported to src/pipeline.py as a reusable module.
...
This is a deliberate choice: rather than imputing hundreds of thousands of missing values before understanding the data, the pipeline starts clean and adds complexity only where it improves signal.
...
- **Log-transformed target**: Raw prices are heavily right-skewed (median ~$27k, long tail past $100k). A `log1p` transformation stabilizes variance and lets models optimize on relative rather than absolute errors. Predictions are converted back to dollars via `expm1` at evaluation time.

#### Import Raw and Display Original Data

The raw dataset contains 762,091 listings across 20 columns. Missing values are concentrated in `price_drop` (46%), `seller_rating` (28%), and `mpg` (19%):
```{python}
...
The cleaning function computes `mpg_avg`, drops incomplete rows, and casts binary flags. Results are cached as a Parquet checkpoint.
```{python}
...
#| fig-cap: "With outliers removed, the distribution is right-skewed but roughly unimodal, peaking around $15k–$20k."
...
#| fig-cap: "Listings grow exponentially from 2015 onward, with 2020 as the most common model year. Pre-2010 vehicles are sparse."
...
#| fig-cap: "Porsche and Land Rover show extreme outliers above $1M. Most brands cluster below $100k."
order = (
    df_cleaned
...
**Toyota, Ford, and Jeep dominate the dataset.** The manufacturer distribution is imbalanced — the top 3 brands each have 15,000+ listings while brands like Jaguar and Mitsubishi have under 3,000. This imbalance means model performance will naturally be better for well-represented brands, and rare brand predictions carry more uncertainty.

**Recent model years are overrepresented.** The year distribution shows exponential growth in listings from 2015 onward, with 2020 being the most common year. This means the models see relatively few examples of older vehicles (pre-2010), which partially explains why residuals tend to increase for vehicles older than ~15 years.
...
**Model count varies widely by manufacturer.** Volkswagen and Toyota each have 350+ unique model names, while Chrysler has under 100. This high cardinality in the `model` column is one reason CatBoost's native categorical handling outperforms Ridge's one-hot encoding approach.
...
The raw `engine` column is a free-text field with inconsistent formatting (e.g., "2.0L I4 Turbo", "V8 Flex Fuel", "3.6L V6 24V MPFI DOHC"). Rather than one-hot encoding 1,818 unique strings, I extracted five structured features: displacement (liters), cylinder count, engine layout (I/V/H), forced induction (turbo/supercharged), and hybrid status.

Missing engine attributes arose from heterogeneous text formats rather than data loss — this is structured missingness due to information compression, not sensor failure. I preserved those rows and handled partial observability via median imputation for numeric attributes and explicit "unknown" categories for nominal attributes.

#### Transmission Normalization

Transmission descriptions contained extensive brand-specific and marketing terminology (255 unique values). I normalized these into transmission type (manual/automatic/CVT) and gear count, which preserved predictive signal while dramatically reducing feature dimensionality.
...
- **Vehicle age**: `REFERENCE_YEAR (2023) - year`. A continuous feature more informative than raw year for depreciation modeling.
...
def build_features_df():
    ...
    # variance and lets the models optimize on relative rather than
    ...
    # (black interiors are overwhelmingly common; brown/beige may
    # correlate with trim level).
    def interior_color_base(c):
        c = str(c).lower().strip()
        if "black" in c or "ebony" in c or "jet" in c:
            return "black"
...
The engineered feature set has 27 columns — down from the raw dataset's high-cardinality text fields:
```{python}
...
1. **Regularized Linear Model (Ridge)**
   ...
   - Helps determine how much signal exists in linear relationships.
...
- **Median Absolute Error** — robust measure of typical error

Because the price distribution contains rare luxury vehicles priced far above the median market value,  
metrics are also reported on a **1–99% clipped subset** based on the true price distribution.
...
The Ridge regression achieved a test MAE of ~$3,013 and RMSE of ~$7,827, serving as a strong interpretable baseline. The gap between MAE and RMSE indicates that while typical pricing errors are modest (~$1,758 median absolute error), a few high-value or atypical vehicles contribute disproportionately to total error.

When clipped to the 1–99% price range ($7k–$91k), Ridge's RMSE drops to $4,214 — nearly half the full-distribution value. This confirms that the linear model handles the bulk of the market well but struggles with rare luxury vehicles where pricing is driven by factors not well-captured by additive features.
...
def get_catboost_training_data(X, y, *, mode="full"):
    ...
    if mode == "smoke":
        ...
        # rate, L2 regularization) that have the largest effect on model
        ...
        # set, optionally refit the final model on train+val combined.
...
**Performance**: CatBoost cuts Ridge's test MAE roughly in half ($1,912 vs. $3,013) and reduces RMSE by nearly 50% ($3,940 vs. $7,827). The clipped test RMSE of $2,659 means that for the vast majority of vehicles, predictions are within ~$2,700 of the actual price — well within the normal range of market variability for used cars.
...
Notably, `mpg_avg` (8.1%) ranks higher than `engine_cylinders` (6.1%), suggesting that fuel efficiency carries meaningful price signal beyond what engine size alone captures. The `luxury_age_interaction` feature (3.2%) confirms that luxury vehicles do follow distinct depreciation patterns.
...
def build_ft_transformer_model(lr_schedule):
    ...
    #   fit outliers harder without improving the bulk of predictions.
    ...
    #   natural "sequence start" and every token carries useful signal.
...
The FT-Transformer was included as evidence of advanced experimentation with deep learning on tabular data, not necessarily as the production pick.
...
**Performance**: The FT-Transformer achieved competitive results — its clipped test MAE ($1,803) is within $20 of CatBoost ($1,783), showing that it learned meaningful representations. However, its full-distribution RMSE ($4,196) is ~6% higher than CatBoost's ($3,940), indicating greater sensitivity to rare, high-price vehicles.
...
**Training behavior**: The training curves show healthy convergence with train and validation loss both decreasing and stabilizing by ~epoch 30. The small gap between them indicates minimal overfitting.

**Why CatBoost still wins**: On this dataset, CatBoost's native categorical handling and ordered boosting provide a structural advantage over learned embeddings. The FT-Transformer requires more data and tuning to match tree-based methods on tabular problems — a finding consistent with the broader ML literature (e.g., Grinsztajn et al., 2022).
...
**Predicted vs. Actual (unclipped)**: The most visible progression across models is how tightly predictions track the diagonal. Ridge shows substantial scatter above ~$100k, with predictions compressing toward the center — a hallmark of linear models that regress toward the mean on extreme values. CatBoost and the FT-Transformer both track the diagonal much more faithfully up through ~$200k, though all three models underpredict the most expensive vehicles ($300k+). This is expected: the log-space loss treats a 2× error equally whether the car costs $20k or $200k, so the models optimize for relative accuracy rather than chasing rare ultra-luxury outliers.

**Residuals vs. Predicted**: Ridge shows a clear fan shape — residual variance increases with predicted price, indicating heteroscedasticity that the linear model cannot capture. CatBoost's residuals are much tighter and more symmetric around zero, with only isolated large positive residuals (underpredictions on high-value vehicles). The FT-Transformer's residual pattern is similar to CatBoost's, though with slightly more spread at the high end.

**Residuals vs. Age**: All three models show residual variance increasing for older vehicles (age > 15), which reflects the dataset's sparse representation of pre-2008 cars. Among young vehicles (age 0–5), CatBoost produces the tightest residual band, confirming its advantage on the most data-dense segment.

**Absolute Error vs. Actual (log scale)**: This is the most revealing diagnostic. For all three models, absolute error grows roughly linearly with actual price on a log scale — errors are proportional to vehicle value, which is the expected behavior from a log-space loss function. CatBoost's error curve sits noticeably below Ridge's across the entire price range, and slightly below the FT-Transformer's at the high end.

**Pred vs. Pred (model agreement)**: The Ridge-vs-CatBoost plots show CatBoost systematically predicting higher than Ridge for expensive vehicles and lower for cheap ones — CatBoost captures the full price range more accurately while Ridge compresses toward the center. The CatBoost-vs-Keras plots show strong agreement along the diagonal with moderate scatter, confirming that both nonlinear models learned similar structure but disagree on individual high-value vehicles. This disagreement pattern suggests an ensemble could potentially improve tail performance.

### Final Model Selection

CatBoost is confirmed as the final model. As detailed in the [Project Background](#project-background), it achieved the lowest error across every metric while also being the most practical to tune and maintain. For a typical used car in the $7k–$91k range, its predictions fall within ~$1,266 of the actual price at the median.

### Key Takeaways

1. **Feature engineering mattered more than model complexity.** The jump from raw features to engineered features (parsed engines, normalized transmissions, color collapsing) provided a larger performance boost than the jump from Ridge to CatBoost. All three models benefited from the same carefully engineered feature set.

2. **CatBoost's categorical handling is a structural advantage.** On a dataset with 5,680 unique models and 1,818 unique engine strings, CatBoost's native categorical support avoids the dimensionality explosion that handicaps linear models.

3. **Luxury vehicles exhibit higher intrinsic price variance** that is not fully explained by structured listing features. This manifests as increasing absolute error at the high end across all model classes — a data reality, not a modeling flaw.

4. **Deep learning is competitive but not dominant on tabular data.** The FT-Transformer came within ~$20 of CatBoost on clipped MAE, confirming that learned embeddings can capture meaningful structure. But CatBoost's combination of native categorical support, built-in regularization, and lower tuning burden makes it the practical choice.

### Limitations and Future Work

- **Geographic features**: The dataset lacks location data (state, ZIP, metro area), which meaningfully affects used-car pricing.
- **Temporal dynamics**: All listings are treated as a cross-section. Time-aware modeling (e.g., how pandemic-era supply constraints affected pricing) could improve accuracy.
...
- **Loss function tuning**: The FT-Transformer's performance on tail observations might improve with a Huber or quantile loss function rather than MSE in log space.

## Conclusion

This project demonstrates that careful feature engineering combined with a well-suited model architecture can produce used-car price predictions accurate enough for real business use — within ~$1,300 of the true price for typical vehicles, across a dataset spanning 29 manufacturers and 5,680 model variants.

The progression from Ridge to CatBoost to the FT-Transformer revealed that the biggest wins came from understanding the data, not from model complexity: collapsing 1,818 raw engine strings into five structured features, normalizing 255 transmission variants into type and gear count, and engineering domain-informed features like luxury-brand depreciation interactions. CatBoost captured these patterns most reliably, and its practical advantages in training speed, tuning stability, and categorical handling make it the right production choice.
