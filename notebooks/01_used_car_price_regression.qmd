...
sns.countplot(x="manufacturer", data=df_cleaned, order=order)
...
sns.countplot(x="year", data=df_cleaned, hue="year", legend=False)
...
plt.figure(figsize=(10, 5))
sns.barplot(data=df_models, x="manufacturer", y="unique_model_count")
plt.xticks(rotation=90)
plt.ylabel("Unique Model Count")
plt.xlabel("Manufacturer")
plt.title("Unique model count per manufacturer", fontsize=14)
plt.tight_layout()
plt.show()
...
sns.boxplot(data=df_cleaned_filtered, x="manufacturer", y="price", order=order)
...
## Save features checkpoint
```{python}
# Save for fast reload later (no need to rerun all parsing)
features_df = X.copy()
features_df["target_log1p_price"] = y.values
...
if RIDGE_PATH.exists():
    print(f"Loading Ridge model: {RIDGE_PATH}")
    best_ridge = joblib.load(RIDGE_PATH)
else:
    print("Training Ridge (no saved model found)...")
    # ... your existing ridge training code here ...
...
# Keep only columns that exist (defensive for dataset changes)
...
```{python}
# Feature space size
best_ridge.named_steps["prep"].fit(X_train)
ohe = best_ridge.named_steps["prep"].named_transformers_["cat"].named_steps["ohe"]
