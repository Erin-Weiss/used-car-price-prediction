...
FT_CFG = {
    # training
    "batch_full": 2048, # instead of 2048, 1024
    "epochs_full": 120,
    "batch_smoke": 512,
    "epochs_smoke": 5,

    # early stopping
    "es_patience_full": 20, #10
    "es_patience_smoke": 3,

    # optimization
    "lr": 2e-4,              # base LR (peak after warmup)
    ...
    # warmup
    "warmup_frac": 0.10,     # 10% of total steps
...
# how long each trial trains (keep short; Step 6 is the real training)
...
def build_model_with_cfg(cfg: dict, *, n_train_rows: int, epochs: int, batch_size: int):
    ...
    # IMPORTANT: your build_ft_transformer_model reads global FT_CFG
...
#    (only tune a few knobs to avoid "breaks")
# ----------------------------------------------------------
def apply_hp_to_cfg(base_cfg: dict, hp: "kt.HyperParameters") -> dict:
    cfg = dict(base_cfg)

    # Keep it tight. Add more later once stable.
    cfg["lr"] = hp.Choice("lr", [1e-4, 2e-4, 3e-4])
    cfg["warmup_frac"] = hp.Choice("warmup_frac", [0.05, 0.10, 0.15])
    cfg["weight_decay"] = hp.Choice("weight_decay", [0.0, 1e-4, 3e-4])

    cfg["ff_dropout"] = hp.Choice("ff_dropout", [0.10, 0.15, 0.20])
    cfg["attn_dropout"] = hp.Choice("attn_dropout", [0.05, 0.10, 0.15])
    cfg["embed_dropout"] = hp.Choice("embed_dropout", [0.00, 0.05, 0.10])

    return cfg


# ----------------------------------------------------------
# 4) Run tuner (reuses get_run_data_for_mode)
# ----------------------------------------------------------
def run_tuner_for_mode(mode: str):
    ...
    trial_batch  = int(FT_CFG["batch_smoke"] if mode == "smoke" else FT_CFG["batch_full"])
    batch_eff = min(trial_batch, int(len(y_train_run)))
    ...
    # Model builder for trials
    def hypermodel(hp):
        cfg_trial = apply_hp_to_cfg(FT_CFG, hp)
    ...
    # Trial callbacks (simple + aligned)
    ...
    print(f"🔎 Tuning START | mode={mode} | trials={TUNER_MAX_TRIALS} | epochs/trial={trial_epochs} | batch={batch_eff}")
    tuner.search(
...
# ----------------------------------------------------------
# 5) Apply tuning results to FT_CFG (only when it matters)
...
if TUNE:
    if TUNE_WHEN in ("smoke", "both") and TRAIN_MODE == "smoke":
        ...
        if best_hp is not None:
            # Apply only the tuned keys
            FT_CFG["lr"] = float(best_hp.get("lr"))
            FT_CFG["warmup_frac"] = float(best_hp.get("warmup_frac"))
            FT_CFG["weight_decay"] = float(best_hp.get("weight_decay"))
            FT_CFG["ff_dropout"] = float(best_hp.get("ff_dropout"))
            FT_CFG["attn_dropout"] = float(best_hp.get("attn_dropout"))
            FT_CFG["embed_dropout"] = float(best_hp.get("embed_dropout"))

            print("✅ FT_CFG updated (for Step 6 full training):")
            print({k: FT_CFG[k] for k in ["lr","warmup_frac","weight_decay","ff_dropout","attn_dropout","embed_dropout"]})
```
